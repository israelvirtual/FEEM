...
% pModel = fitrsvm(x',y','KernelFunction', 'polynomial','PolynomialOrder',2)
% pModel.ModelParameters
% y1 = predict(pModel,EmWaveLength1')
% coeff = [coeff ; polyfit(x,(y1(1:length(x)))',2)]
...
I = imagesc(EmWaveLength1, ExcWaveLength1, 10*log10(average));
set(gca,'YDir','normal')
title('Type number of rays')

numObjects = input('Type number of rays\n')
...
title(['Choose 2 points on the ', num2str(i), 'th biggest rays for correction. Press enter to validate'])
[X Y] = getpts;
correct = [correct; [abs(X(2)-Y(2)) abs(X(2)-Y(2))]];
end
%% Regression 3 fitnlm
